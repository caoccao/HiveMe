...
| Deferred terminal mark-read writes preserve later arrivals and cannot clear replacement history; deterministic regression coverage includes empty and populated startup | [tui.md](tui.md#topic-tree), [session.md](session.md#operations) | `hiveme-core::storage`, `hiveme-core::session`, `crates/hmc/src/tui` | CI reliability | done |
