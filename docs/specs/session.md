...
**Status: specified, not built.** Phase 1 of
[the terminal UI plan](../plans/plan-terminal-ui.md) moves the code that today lives
in `src-tauri/src/{mqtt,controller,notification,config,update,protocol}.rs` into this
module without changing what `hmg` does; the native end-to-end test is what proves
that. Until then the semantics below are the ones [gui.md](gui.md#ipc) describes for
the GUI, and that section stays the IPC contract of `hmg`.
...
Feature `session` of `hiveme-core`, which implies `storage`. Both applications turn
it on, so `hmc` links SQLite from phase 1 on.

```
crates/hiveme-core/src/session/
  mod.rs        Session: open, connect, disconnect, publish, set_config, status, shutdown
...
`ureq`, which the update check needs, moves to `hiveme-core` under the feature. The
rustls provider is still installed once per process by `mqtt::tls`, as
[hivemq-cloud.md](hivemq-cloud.md#transport-and-tls) explains; `hmc` now needs that
too, because it links `ureq`'s `ring` beside `rumqttc`'s `aws-lc-rs`.
...
- `begin_shutdown` refuses new connections and publishes and interrupts startup and
  subscription waits. `shutdown` ends the broker session as
  [hivemq-cloud.md](hivemq-cloud.md#disconnect-and-quit) describes. Both applications
  bound the whole quit at ten seconds and exit even when the broker is unreachable.
...
(`NotConnected`, `Quitting`, `NothingToSend`, `NotJson`), rather than `anyhow`,
...
`SessionEvent` on a `tokio::sync::broadcast` channel. `hmg` forwards each one to the
frontend under the event names of [gui.md](gui.md#events); the terminal UI applies
each one to its state.

| Event | Payload | When |
|-------|---------|------|
| `Status` | `Status` | every connection state change, every reconnect countdown, every pause toggle |
...
One copy of the config in memory behind a lock, so that an operation, the pump, and a
window handler all see the same thing; the rules of `src-tauri/src/config.rs` today.
...
application makes differently. It asks a `Toaster`:
...
| Application | Toaster |
|-------------|---------|
| `hmg` | The Tauri notification plugin on Linux and macOS; `tauri-winrt-notification` with the registered `HiveMe` identity on Windows. Unchanged from today. |
...
The check of `src-tauri/src/update.rs`, unchanged: due when `update.lastChecked` is
...
|---------|---------------------|------------------------------|
| Entry | `lib.rs`: one `#[tauri::command]` per operation, alphabetized, `convert_error` | `mod.rs`: the event loop |
| Events | a task forwarding `SessionEvent` to `app.emit` under the names of [gui.md](gui.md#events) | applied to the application state |
| Toaster | `notification.rs` | `notify.rs` |
| Opening things | the opener plugin for URLs and the config file | the `open` crate |
| Window or screen | `window.rs`: geometry in `gui.window`, the quit path | terminal setup and restore, the quit path |
...
toggle stops the toaster from being called; `shutdown` ends the session within its
bound. A scripted `Toaster` records the calls. The unit tests of the moved code move
with it.
