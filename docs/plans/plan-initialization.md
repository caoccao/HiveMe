...
- Later: `passwordRef.type = "Keychain"` via the `keyring` crate; an `hmc` option (not a subcommand, since a bare word is a message) writes the keychain entry. Encryption keys and the REST token follow the same `*Ref` pattern.
...
- Secret: 32 random bytes, base64url encoded, generated by a future `hmc` key generation option or any tool that can produce 32 random bytes.
...
- Encryption (`A256GCM`, a key generation option, `Opportunistic` and `Required` modes).
...
- Options that show the config, print its path, and store the password in the keychain. `hmc` has no subcommands: `hmc config` is the message `config`. A non-interactive tail of a topic filter is superseded by the terminal UI plan.
